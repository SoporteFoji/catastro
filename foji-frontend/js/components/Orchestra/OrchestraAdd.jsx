...
import { getNewOrchestra, getOrchestra } from '../../utils/get.js';
import {
...
class OrchestraAdd extends Component {
  constructor(props) {
    super(props);
    this.state = {
      editMode: true,
    ...
    this.onCrear = this.onCrear.bind(this);
  ...
  onCrear(data) {
    console.log('Padre');
    const orquesta = data;
    console.table(orquesta);
    orquesta.orchestra_id="";

    const ingresoOrquesta = {
      name: orquesta.name,
      orchestra_type: orquesta.orchestra_type,
      creation_date: orquesta.creation_date,
      city: orquesta.city
    }

    fetch('/api/orquesta-create/', {
      method: 'POST',
      headers: {
        'X-CSRFToken': get_csrf_token(),
      ...
      body: JSON.stringify(ingresoOrquesta),
    })
      .then(response => response.json()
      
      ). then(data => {
        console.log(data);
        const url = '/orquesta/' + data.id; 
        console.log(url);
        window.location = url;
  ...
  render() {
    ...
    // Edit mode: render Orchestra Edit INGRESO ORQUESTa
    } if (edit) {
      return (
        <div className="container mt-3">
          <OrchestraEdit
