...
class OrchestraProfile extends Component {
  ...
  componentDidMount() {
    const self = this;
    axios
      .get(`/api/orchestras/${this.props.orchestraId}/`)
      .then((response) => {
  ...
  deleteOrchestra(id) {
    console.log('jskskl');
    console.log(this.props.orchestraId);
    if (confirm(`Confirma borrar orquesta:${this.state.data.orchestra.name}`)) {
      this.servicio
        .borrarOrquesta(this.props.orchestraId)
        .then((response) => {
