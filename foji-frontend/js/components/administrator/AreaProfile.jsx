...
import { getUser, getAdministrator, getArea } from '../../utils/get.js';
import {
...
class AreaProfile extends Component {
  constructor(props) {
    super(props);
    this.state = {
      editMode: true,
  ...
  componentDidMount() {
    ...
    axios.get(`/api/area/${this.props.areaId}/`)
      .then((response) => {
        console.table(response);
        Area = getArea(response.data);
        self.checkEmptyValues(Area);
        self.setState({
          saving: false,
          data: Area,
        }, self.checkTab);
