...
import ProvinciaForm from './ProvinciaForm.jsx';
import Spinner from '../Spinner.jsx';
import { getArea, getRegiones } from '../../utils/get.js';
import {
...
class ProvinciaProfile extends Component {
  constructor(props) {
    super(props);
    this.state = {
      editMode: true,
  ...
  componentDidMount() {
    ...
    axios.get(`/api/provincia/${this.props.provinciaId}/`)
      .then((response) => {
        console.table(response);
        Provincia = getArea(response.data);
        self.checkEmptyValues(Provincia);
        self.setState({
          saving: false,
          data: Provincia,
        }, self.checkTab);
    ...
    .then((response) => {
      Regiones = getRegiones(response.data);
      self.checkEmptyValues(Regiones);
      self.setState({
        regiones: Regiones,
      }, self.checkTab);
      console.table(this.state.regiones);
    })
    .catch((error) => {
      console.log(error);
      self.setState({ regiones: [] });
    });
...
export default ProvinciaProfile;
