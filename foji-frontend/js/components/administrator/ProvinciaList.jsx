...
class ProvinciaList extends Component {
  constructor(props) {
    super(props);
    this.state = {
      count: 0,
  ...
  getList(query) {
    ...
    let excel;
    if (!query) {
      url = `${'/api/provincia-list/?' + 'page='}${this.state.page}`;
      excel = '/provincia/excel/';
    } else {
      url = `/api/provincia-list/${query}&page=${this.state.page}`;
      excel = `/provincia/excel/${query}`;
    }
    console.log(url);

    axios.get(url)
      .then((response) => {
        AllProvincia = getAreaList(response.data.results);
        self.setState({
      ...
      .catch((error) => {
        ...
        if (error.response.status === 404) {
          console.log('404');
          AllProvincia = getAreaList();
          console.log(AllProvincia);
          self.setState({ provincias: AllProvincia });
        }
      });
  }
  ...
  getListItems() {
    ...
    if (this.state.provincias.length) {
      return (
        this.state.provincias.map((item, i) => {
          if (item.coordinator && Object.keys(item.coordinator).length) {
            item.coordinator = getArea(item.coordinator);
          }
          return (
            <tr key={`provincia-item-${i}-${item.id}`}>
              <TableItemCustom {...item} type="provincia-admin" role="provincia-admin" />
            </tr>
          );
        })
      );
      ...
      <>
        ...
        <div className="row">
          <div className="col-12 text-sm-right">
            <a className="btn btn-light mt-5 d-block d-sm-inline-block text-secondary" href="/provincias/excel/">Descargar todo</a>
          </div>
        </div>
