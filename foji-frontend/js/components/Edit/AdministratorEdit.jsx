...
class AdministratorEdit extends Component {
    ...
    if(type){
      switch(type){
        case 'social_networks':
          let targetSN = e.name.substring(e.name.indexOf('-') + 1);
  ...
  onSave(){
    ...
    console.log(this.props[this.state.currentTab]);
  ...
  validateForms(tab){
    ...
    if(this.forms[tab] && this.forms[tab] !== undefined){
      let inputs = this.forms[tab].querySelectorAll('input, select');
      switch(tab){
        case "orchestra":
  ...
  showModal(){
    ...
    if(this.state.modal){
      if(this.state.error === 'save needed'){
        return(
          <Modal title='Cambios no aplicados' 
            onCloseModal={() => this.setState({ modal: false })} 
            onSubmit={this.onSave} size="sm" center>
            <div className="p-3">
              Hay cambios que no estan guardados.<br />
        ...
        return(
          <Modal title='Errores en Formulario' 
            onCloseModal={() => this.setState({ modal: false })} 
            onSubmit={() => this.setState({ modal: false })} buttonTitle="Aceptar" size="md" center>
            <div className="p-3">
            ...
            <div className="row">
              <div className="col-sm-6 text-center text-sm-left mb-3 mb-sm-0">
                <button className="btn btn-light" type="button" onClick={
                  ...
                  e => this.cancelEdit()} >Cancelar</button>
              </div>
