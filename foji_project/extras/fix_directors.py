...
def fix_coordinator_phone_number_data(workbook, row, debug=True):
    ...
    print(data)
    coordinator_username = data.get('director_email')
    phone_number = data.get('director_phone_number_mobile')
    print('Phone number for {}: {}'.format(coordinator_username, phone_number))

    if not coordinator_username:
        return False, 'ERROR {}: No hay email de director.'.format(row)

    # Find director.
    directors = Director.objects.filter(personal_information__email__iexact=coordinator_username)
    if len(directors) == 0:
        return False, 'ERROR {}: No existen directores con email {}.\n'.format(
            row,
