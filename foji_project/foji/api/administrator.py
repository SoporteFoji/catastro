...
from ..serializers.orchestra import OrchestraSerializer
...
class AdministratorUpdateSerializer(serializers.ModelSerializer):
    ...
    def update(self, instance, validated_data):
        admin = Administrator.objects.get(pk=validated_data['id'])
        user = User.objects.get(user=validated_data['user'])
        #personal = PersonalInformation.objects.get(id=)
        admin = Administrator.objects.get(user=validated_data['user'])
        admin.personal_information = validated_data['personal_information']
        print(admin)
        #print(admin)
        return instance
...
class PersonalInformationDetail(APIView):
    def get(self, request, format=None):
        personalinformation = PersonalInformation.objects.all()
        serializers = PersonalInformationSerializer(personalinformation, many=True)
        return Response(serializers.data)

class PersonalInformationViewSet(viewsets.ModelViewSet):
    serializers = PersonalInformationSerializer
...
class AdministratorList(APIView):
    def get(self, request, format=None):
        ...
        serializers = AdministratorSerializer(administrator, many=True)
        return Response(serializers.data)
...
class AdministratorDelete(APIView):
    permission_classes = (permissions.IsAuthenticatedOrReadOnly,)
