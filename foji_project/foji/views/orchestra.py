...
class Delete(DetailView):
    model = Orchestra
    template_name = 'foji/dashboard/orchestra_del.html'

    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)

        # By default no one can edit the orchestra.
        context['can_del'] = 'false'
        ...
        if administrator is not None and self.request.user.is_staff is False:
            context['can_del'] = 'true'

        print(context['can_del'])
        # Check if user is a coordinator.
        try:
            coordinator = user.coordinator
        except:
...
class Creacion(CreateView):
    model = Orchestra
    fields = '__all__'
    template_name = 'foji/dashboard/orchestra_add.html'
